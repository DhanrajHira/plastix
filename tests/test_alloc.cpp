...
using ParticleAllocator = plastix::alloc::PageAllocator<Particle>;

TEST(PageAllocatorTest, AllocateAndReadBack) {
  ParticleAllocator Alloc(16);

  plastix::alloc::AllocId<Particle> Ids[5];
  for (int I = 0; I < 5; ++I) {
    Ids[I] = Alloc.Allocate();
    ASSERT_NE(Ids[I], static_cast<size_t>(-1));
  }
  ...
  for (int I = 0; I < 5; ++I) {
    auto &P = Alloc.Get(Ids[I]);
    P.X = static_cast<float>(I);
    P.Y = static_cast<float>(I * 2);
    P.Z = static_cast<float>(I * 3);
    P.Mass = static_cast<float>(I) + 0.5f;
  }

  for (int I = 0; I < 5; ++I) {
    auto &P = Alloc.Get(Ids[I]);
    EXPECT_FLOAT_EQ(P.X, static_cast<float>(I));
    EXPECT_FLOAT_EQ(P.Y, static_cast<float>(I * 2));
    EXPECT_FLOAT_EQ(P.Z, static_cast<float>(I * 3));
    EXPECT_FLOAT_EQ(P.Mass, static_cast<float>(I) + 0.5f);
  }
}

TEST(PageAllocatorTest, AllocateAtCapacity) {
  ParticleAllocator Alloc(3);
